...
import { updateForm, verifyDomainDNS } from "../../../actions";
...
export default function EditFormPanel({ form }: { form: FormRow }) {
  ...
  const [activeTab, setActiveTab] = useState<"content" | "style" | "fields" | "domain">("content");
  ...
  const [customFont, setCustomFont] = useState(form.custom_font || "Inter");
  const [customDomain, setCustomDomain] = useState(form.custom_domain || "");
  const [verifyingDomain, setVerifyingDomain] = useState(false);
  const [dnsStatus, setDnsStatus] = useState<{ valid: boolean; error?: string } | null>(null);
  ...
  async function handleVerifyDNS() {
    ...
    setDnsStatus(null);
    const res = await verifyDomainDNS(customDomain.trim());
      ...
      <div className="mb-8 flex flex-wrap items-center justify-between gap-4 border-b border-[#ECE7E0] pb-6">
        <div>
          ...
          <p className="mt-1 text-sm text-[#6B6B6B]">
            Configure and customize your testimonials collection form.
          </p>
        </div>
          ...
          <div className="rounded-2xl border border-[#ECE7E0] bg-white p-6 shadow-sm min-h-[380px]">
            ...
            {activeTab === "style" && (
              <div className="flex flex-col gap-6">
                ...
                <div className="flex flex-col gap-1.5 border-t border-[#ECE7E0]/60 pt-5">
                  <label className="text-xs font-bold text-[#1A1A1A] uppercase tracking-wider">Custom Font</label>
                  <select
                    value={customFont}
                  ...
                  >
                    ...
                    <option value="Plus Jakarta Sans">Plus Jakarta Sans (SaaS Standard)</option>
                  </select>
                  <p className="text-[11px] text-[#8A8A8A]">Select the primary typography family for your collection form.</p>
                </div>

                {/* Custom CSS Text Area */}
                <div className="flex flex-col gap-1.5 border-t border-[#ECE7E0]/60 pt-5">
                  <label className="text-xs font-bold text-[#1A1A1A] uppercase tracking-wider">Custom CSS</label>
                  <textarea
                    rows={6}
                    value={customCss}
                    onChange={(e) => setCustomCss(e.target.value)}
                    placeholder="/* Example: change submit button font size */&#10;button[type='submit'] {&#10;  font-size: 16px;&#10;}"
                    className={`${inputClass} font-mono text-xs`}
                  />
                  <p className="text-[11px] text-[#8A8A8A]">Paste custom CSS rules to override default collection form styles.</p>
                </div>
              </div>
            )}
            ...
            {activeTab === "domain" && (
              <div className="flex flex-col gap-5">
                <div className="border-b border-[#ECE7E0]/60 pb-3">
                  <h3 className="text-sm font-bold text-[#1A1A1A]">Custom Domain White-Labeling</h3>
                  <p className="mt-1 text-xs text-[#6B6B6B]">White-label your collection form under your own custom brand domain.</p>
                </div>

                <div className="flex flex-col gap-2">
                  <label className="text-xs font-bold text-[#1A1A1A] uppercase tracking-wider">Domain Name</label>
                  <div className="flex gap-2">
                    <input
                      ...
                      placeholder="e.g. reviews.acmedental.com"
                    ...
                    {customDomain.trim() && (
                      <button
                        ...
                        className="px-4 py-2 bg-[#1A1A1A] hover:bg-[#333333] disabled:opacity-50 text-white rounded-xl text-xs font-bold transition-all cursor-pointer"
                      >
                  ...
                  {dnsStatus && (
                    <div className={`flex items-start gap-2.5 p-3 rounded-xl border text-xs transition-all duration-300 ${
                      dnsStatus.valid 
                        ? "bg-[#F3FBF7] border-[#CDEEDB] text-[#1E6B45]" 
                        : "bg-[#FFF9F2] border-[#FFE2C5] text-[#A05E1C]"
                    }`}>
                      {dnsStatus.valid ? (
                        <>
                          <CheckCircle2 size={15} className="mt-0.5 shrink-0 text-[#2E9E6B]" />
                          <div>
                            <span className="font-bold">DNS configured correctly!</span>
                            <p className="mt-0.5 text-[10px] opacity-80">Your CNAME record successfully points to vercel-dns.com.</p>
                          </div>
                        </>
                      ) : (
                        <>
                  ...
                  <ol className="list-decimal pl-4 text-[11px] text-[#6B6B6B] flex flex-col gap-1">
                    <li>Add a <strong>CNAME</strong> record pointing to <code>cname.vercel-dns.com</code> in your domain registrar DNS settings.</li>
                    <li>Ensure the custom domain is registered in the Blovi Vercel project configuration.</li>
                  </ol>
                </div>
              </div>
            )}
