...
/* ═══════════════════════════════════════════════════════════
   Mobile polish-demo quotes (unchanged)
   ═══════════════════════════════════════════════════════════ */
const QUOTES = [
  {
...
export default function LandingHero() {
  ...
  /* ── Mobile polish interaction (unchanged) ── */
  const swapNext = () => {
    phaseRef.current = "swapping";
    const advance = () => setQuoteIndex((i) => (i + 1) % QUOTES.length);
    if (prefersReducedMotion()) { advance(); return; }
    gsap.to(cardRef.current, { autoAlpha: 0, y: -14, duration: 0.4, ease: "power2.in", onComplete: advance });
  };
  ...
  const polish = () => {
    ...
    const tl = gsap.timeline({
      ...
      onComplete: () => { phaseRef.current = "polished"; timersRef.current.next = setTimeout(swapNext, 3200); },
    });

    tl.to(section.querySelector(".hero-polish-btn"), { scale: 0.92, duration: 0.12, yoyo: true, repeat: 1, ease: "power2.inOut" })
      .to(section.querySelector(".hero-label-before"), { autoAlpha: 0, y: -8, duration: 0.3 }, 0.1)
      .fromTo(section.querySelector(".hero-label-after"), { autoAlpha: 0, y: 8 }, { autoAlpha: 1, y: 0, duration: 0.4 }, 0.3)
      .to(quoteRef.current, { duration: 1.3, scrambleText: { text: `\u201c${q.clean}\u201d`, chars: "lowerCase", speed: 0.6 } }, 0.15);
  };

  // Reset after each mobile quote swap
  useEffect(() => {
    ...
    if (prefersReducedMotion()) return;
    gsap.fromTo(cardRef.current, { autoAlpha: 0, y: 14 }, { autoAlpha: 1, y: 0, duration: 0.5, ease: "power3.out" });
  ...
  useIsoLayoutEffect(() => {
    ...
    return () => { clearTimeout(timers.auto); clearTimeout(timers.next); ctx.revert(); };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  return (
    ...
    >
      ...
      <div className="relative z-20 mx-auto flex w-full max-w-[680px] flex-col items-center text-center pointer-events-none">
        ...
        {/* Live polish demo — mobile/tablet fallback only */}
        <div
          ref={cardRef}
          aria-live="polite"
          className="mt-14 w-full max-w-[640px] rounded-2xl border border-[#ECE7E0] bg-white p-7 text-left shadow-[0_18px_50px_rgba(26,26,26,0.07)] md:p-9 md:hidden pointer-events-auto"
        >
